"""Loads Workflow_<n>'s ``scenario_XX/signal_log.json`` fixtures.

Same simplest-shape precedent as Renewal Remarketing/Diligent Search:
every scenario is a single, already-structured JSON snapshot (no raw
emails, no new extraction target). Still doesn't fit
``src/fixtures/loader.py`` (glob mismatch), so it's workflow-owned, same
precedent as every prior E&S workflow's loader. See
DATA_AND_FIXTURES.md's Workflow_18 layout note.
"""
...
def load_scenario(n: int, scenario_ref: str) -> dict[str, Any]:
    """Loads one scenario's ``signal_log.json``. Raises
    ...
    path = dataset / scenario_ref / "signal_log.json"
    if not path.is_file():
        raise FileNotFoundError(f"no signal_log.json at {path}")
